/* ================================================================================================
Purpose:
    Enables all foreign key, check constraints and triggers.
...
-- work through cursor
-- build command to enable constraint and then execute
OPEN @constraint_list;
FETCH NEXT FROM @constraint_list INTO @table_schema, @table_name, @fk_name;

WHILE ( @@fetch_status = 0 )
BEGIN
    SET @sql_cmd = N'ALTER TABLE ' + @table_schema + N'.' + @table_name + N' CHECK CONSTRAINT ' + @fk_name;
...
-- work through cursor
-- build command to enable triggerand then execute
OPEN @trigger_list;
FETCH NEXT FROM @trigger_list INTO @table_schema, @table_name, @trigger_name;

WHILE ( @@fetch_status = 0 )
BEGIN
    SET @sql_cmd = N'ALTER TABLE ' + @table_schema + N'.' + @table_name + N' ENABLE TRIGGER ' + @trigger_name;
