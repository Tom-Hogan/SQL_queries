...
Notes:
    Contains commented out predefined WHERE clause to filter results for a specific user.
...
================================================================================================ */
WITH
cte_user_details ( sql_output, db_principal_name, sql_sort ) AS
(
    /*
        database user
    */
    SELECT  CASE
                ...
                ELSE
                    'CREATE USER ' + quotename(dp.name) 
                        + ' FOR LOGIN ' + quotename(suser_sname(dp.sid)) 
    ...
    AND     dp.principal_id > 4         /* exclude default SQL users */
    UNION
    /*
        orphaned users
    */
    SELECT      'ALTER USER ' + quotename(dp.name) + ' WITH LOGIN = ' + quotename(dp.name) AS sql_output,
                dp.name                                                                    AS db_principal_name,
                1.1                                                                        AS sql_sort
    FROM        sys.database_principals AS dp
    ...
    */
    ...
    WHERE   du.principal_id > 4 /* exclude default SQL users */
    /*
    ...
    */
    ...
    SELECT  CASE
                ...
                ELSE 'GRANT'
            END
                + ' ' + p.permission_name + ' TO ' + quotename(user_name(pr.principal_id))COLLATE DATABASE_DEFAULT 
                +   CASE
                        WHEN p.state <> 'W'
                            THEN ''
    ...
    WHERE   pr.type IN ('U', 'S', 'G')  /* windows users, sql users, windows groups */
    AND     pr.principal_id > 4         /* exclude default SQL users */
    AND     p.major_id = 0
    /*
        schema permissions
    */
    ...
    SELECT  CASE
                ...
                ELSE 'GRANT'
            END
                + ' ' + p.permission_name + ' ON ' + p.class_desc + '::' COLLATE DATABASE_DEFAULT + quotename(s.name)
    ...
    */
    ...
    SELECT      CASE
                    ...
                    ELSE 'GRANT'
                END
                    + ' ' + p.permission_name + ' ON ' + quotename(s.name) + '.' + quotename(o.name)
                    +   CASE
                            WHEN c.column_id IS NULL
                                THEN space(0)
                            ELSE '(' + quotename(c.name) + ')'
                        END
                    + ' TO ' + quotename(pr.name)COLLATE DATABASE_DEFAULT
                    +   CASE
                            WHEN p.state <> 'W'
                                THEN ''
                            ELSE ' WITH GRANT OPTION'
...
SELECT      u.sql_output,
            u.db_principal_name,
            u.sql_sort
FROM        cte_user_details AS u
            /*
            === to get specific user ===
            */
--WHERE       u.db_principal_name = 'name'
ORDER BY    u.sql_sort,
            u.db_principal_name;
